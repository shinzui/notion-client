...
- [x] (2026-09-15) Final: CHANGELOG `## Unreleased` entries written; live checks run (comment lifecycle, async page create and markdown update, meeting-notes query validation; meeting-note creation skipped by design); Outcomes & Retrospective filled; ADRs 3 and 4 written.
...
- Overall (2026-09-15): all four milestones are complete in four commits (`def4f47`, `1788b1f`, `420beaf`, `6e4cb76`). `cabal build all` builds both packages with no new warnings in the edited modules, and `cabal test` reports `All 232 tests passed` (198 before): 9 `Comment mutation (EP-3)`, 11 `Async tasks (EP-3)` and 14 `Meeting notes (EP-3)`. Eight new `Methods` fields have matching effectful constructors and interpreter cases.
- What remains: live creation of a meeting note (it needs a real recording) and a live meeting-notes query that returns results (it needs a workspace plan with AI meeting notes). `CommentResponse`'s partial branch has not been seen live.
- Lessons: live checks paid for themselves twice. Offline tests could not catch the 202 status rejection, because `FakeNotion` bypasses servant's status check, and they could not catch the invalid relative date string, which the JS SDK types leave as `string`. Notion validating the body before the plan check made encoder validation possible even without the feature. Durable conclusions went into ADR 3 (async operations) and ADR 4 (full-or-partial responses and request-only types).
...
When this plan was written there was no `docs/adr/` directory. By implementation time EP-1 and EP-2 had added [docs/adr/1-tolerant-response-decoders.md](../adr/1-tolerant-response-decoders.md), which requires an unknown fallback on every enum decoded from a response (followed here by `UnknownSurface`, `UnknownAsyncTaskStatus` and the reuse of `UnknownMeetingNotesStatus`). They also added [docs/adr/2-client-runtime-retry-policy-and-typed-errors.md](../adr/2-client-runtime-retry-policy-and-typed-errors.md), which owns `APIErrorCode`. This plan's durable decisions are recorded in [docs/adr/3-background-operations-accept-200-or-202-and-return-asyncor.md](../adr/3-background-operations-accept-200-or-202-and-return-asyncor.md) and [docs/adr/4-full-or-partial-responses-and-request-only-types.md](../adr/4-full-or-partial-responses-and-request-only-types.md).
...
Integration with sibling plans: EP-2 (`docs/plans/7-add-a-configurable-client-runtime-with-retries-typed-error-codes-and-oauth.md`) may replace `AsyncTaskError.code :: Text` with `APIErrorCode`; whichever lands second makes that switch. EP-1 (`docs/plans/6-fix-wire-format-decoding-and-encoding-bugs-found-against-the-official-sdk.md`) owns the meeting-notes payload sub-types in `Notion.V1.BlockContent`. `Notion.V1.MeetingNotes` imports and re-exports them, so the dependency runs from `MeetingNotes` to `BlockContent` and never the other way. (Revised during implementation: an older sentence here had the direction reversed.) EP-6 may type webhook `file_import_result` data with `AsyncTask`.


Revision 2026-09-15 (implementation): Implemented all four milestones and filled in Progress, Surprises & Discoveries, the Decision Log and Outcomes & Retrospective. Three parts of the plan body were revised to match what shipped. The async page routes now use `AsyncVerb`, because Notion returns 202 and a plain `Post`/`Patch` route rejects it. `mnCreatedWithinPast` sends `"custom"`, because Notion rejects `"today"` in a range condition. The final Interfaces paragraph now states the correct import direction between `MeetingNotes` and `BlockContent`. Beyond the plan, `AsyncTaskError.code` uses EP-2's `APIErrorCode`, `Notion.V1.MeetingNotes` re-exports EP-1's payload types, and three request-level tests were added. The Context section now cites ADRs 1–4.
